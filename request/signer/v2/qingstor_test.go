...
func TestQingStorSignerWriteSignature(t *testing.T) {
	...
	err = s.WriteSignature(CanonicalReqByPath(httpRequest))
	assert.Nil(t, err)
...
func TestQingStorSignerWriteQuerySignature(t *testing.T) {
	...
	err = s.WriteQuerySignature(CanonicalReqByPath(httpRequest), 3600)
	assert.Nil(t, err)
...
func TestQingStorSigner_WriteQuerySignatureWithDisposition(t *testing.T) {
	...
	err := s.WriteQuerySignature(CanonicalReqByPath(httpRequest), 3600)
	assert.Nil(t, err)
	targetURL := "https://qingstor.com?response-content-disposition=attachment%3B+filename%3D%22%25E4%25B8%25AD%25E6%2596%2587.jpg%22%3B+filename%2A%3Dutf-8%27%27%25E4%25B8%25AD%25E6%2596%2587.jpg&access_key_id=ENV_ACCESS_KEY_ID&expires=3600&signature=XPMfomqMYu7ej62eqitVcPJyZMvfi72ZxXwGpPWv07I="
	assert.Equal(t, httpRequest.URL.String(), targetURL)
}
...
func TestQingStorSinger_WriteSignatureWithVirtualStyle(t *testing.T) {
	...
	err = s.WriteSignature(CanonicalReqByVhost(httpRequest, "bucket-name"))
	assert.Nil(t, err)
